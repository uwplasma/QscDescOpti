...
for i in range(3):
    ...
    plt.hist(actual_diffs[:, i], bins=50, alpha=0.5, label=f'Actual {diff_names[i]}', color='blue', edgecolor='black')
    plt.hist(predicted_diffs[:, i], bins=50, alpha=0.5, label=f'Predicted {diff_names[i]}', color='orange', edgecolor='black')
    plt.xlabel(f"{diff_names[i]}")
    plt.ylabel("Frequency")
    plt.title(f"Distribution of Actual vs. Predicted {diff_names[i]}")
    plt.legend()
    plt.savefig(f"{diff_names[i]}_distribution_comparison.png")
    print(f"{diff_names[i]} distribution plot saved to '{diff_names[i]}_distribution_model2_comparison.png'.")
    plt.show()

diff_names = ['iota_diff', 'L_grad_B_min_diff', 'beta_diff']
for i in range(3):
    plt.figure(figsize=(10, 5))
    plt.plot(actual_diffs[:, i], alpha=0.5, label=f'Actual {diff_names[i]}', color='blue')
    plt.plot(predicted_diffs[:, i], alpha=0.5, label=f'Predicted {diff_names[i]}', color='orange')
    plt.xlabel(f"{diff_names[i]}")
    plt.ylabel("Frequency")
    plt.title(f"Distribution of Actual vs. Predicted {diff_names[i]}")
    plt.legend()
    plt.savefig(f"{diff_names[i]}.png")
    print(f"{diff_names[i]} distribution plot saved to '{diff_names[i]}_distribution_model2_comparison.png'.")
    plt.show()


# Define tolerances for each diff type
iota_tolerance = 5  # Larger tolerance for iota_diff
other_tolerance = 0.2  # Standard tolerance for the other diffs

# Initialize counters for each difference type
accurate_iota_count = 0
accurate_L_grad_B_min_count = 0
accurate_beta_count = 0
...
for actual, predicted in zip(actual_diffs, predicted_diffs):
    ...
    L_grad_B_min_accuracy = abs(predicted[1] - actual[1]) <= other_tolerance 
    beta_accuracy = abs(predicted[2] - actual[2]) <= other_tolerance 
    
    # Update counts for each diff type
    accurate_iota_count += iota_accuracy
    accurate_L_grad_B_min_count += L_grad_B_min_accuracy
    accurate_beta_count += beta_accuracy
...
beta_accuracy_percentage = (accurate_beta_count / total_count) * 100
...
print(f"  L_grad_B_min_diff accuracy (tolerance = {other_tolerance}): {L_grad_B_min_accuracy_percentage:.2f}%")
print(f"  beta_diff accuracy (tolerance = {other_tolerance}): {beta_accuracy_percentage:.2f}%")
